...
When a fenced code example starts getting too wide, break it over multiple lines instead of letting one long line do all the work.

That usually means:

- split long command sequences into one command per line when possible
- prefer readable flag layout over dense one-liners
- keep examples easy to scan for a tired human in a narrow editor pane

The point is readability first, not showing off that a shell technically allows everything on one line.
